...
— regenerated 2026-08-31 (backend-issues #11). Regenerate by walking that directory if the schema changes again.
...
| `address` | Small Text |  |
| `date_of_birth` | Date |  |
| `age` | Int | read-only — Computed from Date of Birth -- read-only. |
| `height` | Data |  |
| `weight` | Data |  |
...
| `passport_issue_date` | Date | read-only — Computed as Passport Expiry Date minus 5 years -- read-only, no manual entry. |
| `passport_expiry_date` | Date |  |
| `passport_issue_place` | Data |  |
| `passport_scan` | Attach |  |
| `photograph` | Attach Image |  |
| `medical_status` | Select | Pending / FIT / UNFIT — default=Pending — Informational only — NOT required/gated at Registration (2026-08-29 correction). The real medical fitness gate happens post-contract on Placement.medical_selected_status (Selected -> Processing). |
...
| `fee_required` | Check | Toggle on only if this applicant owes/receives a registration-related fee. Everything below stays hidden until this is checked. |
| `fee_type` | Select | Registration Fee / Processing Fee / Visa Fee / Other |
| `fee_direction` | Select | Income / Expense — default=Income |
| `registration_fee_amount` | Currency | fee_currency |
| `fee_currency` | Select | ETB / SAR / KWD / USD / AED / QAR — default=ETB |
| `fee_status` | Select | Pending / Paid / Expired / Refunded — default=Pending — Set to Paid to log this into the Finance ledger (Applicant Transaction) -- happens automatically on save, or use the Log Fee button. |
| `fee_payment_date` | Date |  |
| `fee_transaction` | Link | Applicant Transaction — read-only — Set automatically once this fee has been logged into the Finance ledger -- read-only, links to the actual Applicant Transaction row. |
| `fee_notes` | Small Text |  |
| `fee_log` | Table | Applicant Fee Log |
...
Roles with any access: Admin, Clearance Officer, Communication Manager, Complaint Manager, Finance Manager, Kuwait Embassy, Kuwait LMIS, Kuwait Telesign, Manager, Registrar, Saudi Embassy, Saudi LMIS, Saudi Taeshir, System Manager
...
| `transaction_type` | Select | Income / Expense — required |
| `amount` | Currency | required |
| `currency` | Select | ETB / SAR / KWD / USD / AED / QAR — default=ETB |
| `status` | Select | Not Logged / Pending / Approved / Rejected / Voided — read-only, default=Not Logged — Mirrors the linked Applicant Transaction's status -- refreshed automatically on every Applicant save. Not Logged means the row hasn't been submitted to Finance yet (still blank amount/description or newly added this save). |
| `transaction` | Link | Applicant Transaction — read-only — Set automatically once this row has been logged into the Finance ledger -- read-only, links to the actual Applicant Transaction row for approval. |
| `receipt_url` | Attach | Upload here -- automatically moved to Cloudflare R2 on save. |
...
| Field | Type | Options / Notes |
|---|---|---|
| `applicant` | Link | Applicant — read-only — Optional — set for registration-fee entries logged before any Placement exists (Applicant.log_fee_transaction). Most transactions are traceable via placement instead. |
...
| `receipt_image` | Attach | Upload here -- automatically moved to Cloudflare R2 (agency/{applicant}/finance-receipts/) on save, this field then holds the R2 URL, not a local Frappe file. |
...
| `remark` | Small Text |  |
| `receipt_url` | Attach | Upload here -- automatically moved to Cloudflare R2 (agency/{applicant}/finance-receipts/) on save, this field then holds the R2 URL, not a local Frappe file. |
...
| Field | Type | Options / Notes |
|---|---|---|
| `use_mock_parsing` | Check | default=1 — When enabled, contract/visa/injaz/passport parsing endpoints return fixed mock data instead of running real PDF/OCR extraction. Intended for frontend/dev testing when real documents or OCR dependencies aren't available — leave off in production. |
...
| Field | Type | Options / Notes |
|---|---|---|
| `currency` | Select | SAR / KWD / USD / AED / QAR — required — ETB isn't listed -- it's Birr itself, always 1:1, hardcoded in finance_engine.get_fx_rate rather than recorded here. |
...
| `medical_2_examination_date` | Date |  |
...
Roles with any access: Admin, Clearance Officer, Communication Manager, Complaint Manager, Contract Parser, Finance Manager, Kuwait Embassy, Kuwait LMIS, Kuwait Telesign, Manager, Saudi Embassy, Saudi LMIS, Saudi Taeshir, System Manager, Ticketer
...
Roles with any access: Admin, Clearance Officer, Complaint Manager, Finance Manager, Manager, Registrar, System Manager, Ticketer
