...
async function run() {
  ...
  // Let's call update_musaned_status
  const up = await req('/api/method/applicant_processing.applicant_processing.doctype.applicant.applicant.update_musaned_status', 'POST', {
    applicant: 'APP-00003',
    musaned_status: 'Verified',
    musaned_reference_no: 'MUS-12345',
    is_uploaded_to_musaned: 1
  }, cookies);
  console.log('update_musaned_status response:', up.body);

  // Now call generate_cv in the SAME session
  ...
  console.log('generate_cv response:', gen.status, gen.body);
}
